...
import { User as UserIcon, Search, Settings, AlertCircle, MapPin } from 'lucide-react';
        ...
        <form onSubmit={handleAuth} className="space-y-4">
          {authMode === 'register' && (
            <>
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-400 uppercase tracking-widest">المحافظة</label>
                <div className="relative">
                  ...
                  <input 
                    ...
                    required
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl py-3 pl-12 pr-4 focus:ring-2 focus:ring-indigo-100 text-slate-900 transition-all"
                    placeholder="مثال: حمص، دمشق، حلب..."
                    value={authForm.province || ''}
                    onChange={(e) => setAuthForm({...authForm, province: e.target.value})}
                  />
                </div>
              </div>

              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-400 uppercase tracking-widest">اسم الشارع</label>
                <div className="relative">
                  <MapPin className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                  <input 
                    type="text" 
                    required
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl py-3 pl-12 pr-4 focus:ring-2 focus:ring-indigo-100 text-slate-900 transition-all"
                    placeholder="مثال: شارع الشكري، بناية النور"
