...
import { Product, Order, Vendor, Customer, Category, HeroImage, Deal } from '../types';

interface AdminPanelProps {
  adminSubView: 'products' | 'orders' | 'vendors' | 'customers' | 'reports' | 'hero' | 'deals';
  setAdminSubView: (view: 'products' | 'orders' | 'vendors' | 'customers' | 'reports' | 'hero' | 'deals') => void;
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 bg-slate-50 p-2 rounded-2xl border border-slate-100 overflow-x-auto no-scrollbar">
        ...
        <button 
          onClick={() => setAdminSubView('deals')}
          className={`flex items-center gap-2 px-6 py-3 rounded-xl font-bold text-sm transition-all ${adminSubView === 'deals' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-100' : 'text-slate-900 hover:bg-slate-100'}`}
        >
          <Tag className="w-4 h-4" /> العروض والتخفيضات
        </button>
      </div>
      ...
      {adminSubView === 'deals' && (
        <div className="space-y-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h2 className="text-3xl font-bold text-slate-900">إدارة العروض والتخفيضات</h2>
              <p className="text-slate-900 mt-1">إضافة وعرض وتعديل العروض والخصومات النشطة وكوبونات الخصم للمتجر</p>
            </div>
            <button 
              onClick={() => {
                setEditingDeal(null);
                setNewDeal({ title: '', description: '', discountPercentage: 10, code: '', image: '', isActive: true, productId: '' });
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {deals.map(deal => (
              <div key={deal.id} className="bg-white rounded-[32px] overflow-hidden border border-slate-100 shadow-md flex flex-col justify-between group">
                <div className="relative h-48 overflow-hidden bg-slate-50">
                  <img 
                    src={deal.image || 'https://images.unsplash.com/photo-1607082348824-0a96f2a4b9da?auto=format&fit=crop&q=80&w=400'} 
                    alt={deal.title} 
                    className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105"
                    referrerPolicy="no-referrer"
                  />
                  <div className="absolute top-4 right-4 bg-rose-600 text-white font-black text-xs px-3 py-1.5 rounded-xl shadow">
                    %خصم {deal.discountPercentage}
                  </div>
                  <div className="absolute top-4 left-4 bg-slate-900/80 backdrop-blur-sm text-white font-bold text-[10px] px-2.5 py-1 rounded-lg">
                    {deal.isActive ? '✅ نشط' : '❌ غير نشط'}
                  </div>
                </div>

                <div className="p-6 space-y-3 flex-1 text-right flex flex-col justify-between">
                  <div className="space-y-2">
                    ...
                    {/* Linked Product Info */}
                    {deal.productId && (
                      <div className="text-xs text-indigo-600 font-bold bg-indigo-50/70 p-2.5 rounded-xl flex items-center justify-between border border-indigo-100/30">
                        <span className="font-sans text-slate-900">
                          {products.find(p => p.id === deal.productId)?.name || 'المنتج مفقود'}
                        </span>
                        <span className="text-indigo-600">🔗 المنتج المرتبط:</span>
                      </div>
                    )}

                    {deal.code && (
                      <div className="flex items-center gap-1.5 pt-1 text-xs justify-end">
                        <span className="font-mono bg-slate-100 border border-slate-200 px-2 py-1 rounded-md font-extrabold text-indigo-600">
                          {deal.code}
                        </span>
                        <span className="text-slate-400">كود الخصم:</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-2 pt-4 border-t border-slate-100 justify-end">
                    <button 
                      ...
                      className="text-xs bg-slate-50 hover:bg-slate-100 text-slate-700 font-bold px-3 py-2 rounded-xl border border-slate-200 transition-all flex items-center gap-1.5"
                    >
                      <FileText className="w-3.5 h-3.5" /> تعديل
                    </button>
                    <button 
                      onClick={() => deleteDeal(deal.id)}
                      className="text-xs bg-red-50 hover:bg-red-100 text-red-600 font-bold px-3 py-2 rounded-xl border border-red-100 transition-all flex items-center gap-1.5"
                    >
      ...
      {isDealModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <motion.div 
            initial={{ opacity: 0, scale: 0.95, y: 20 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            className="bg-white rounded-[32px] w-full max-w-xl overflow-hidden shadow-2xl text-right"
          >
            <div className="p-8 border-b border-slate-100 flex items-center justify-between">
              <h3 className="text-2xl font-bold text-slate-900">
                {editingDeal ? 'تعديل العرض الترويجي' : 'إضافة عرض ترويجي جديد'}
              </h3>
              <button onClick={() => setIsDealModalOpen(false)} className="p-2 hover:bg-slate-50 rounded-xl transition-colors">
                <X className="w-6 h-6 text-slate-400" />
              </button>
            </div>
            ...
            <div className="p-8 space-y-5 max-h-[70vh] overflow-y-auto no-scrollbar">
              <div className="space-y-1.5">
                <label className="text-sm font-bold text-slate-900 block">عنوان العرض</label>
                <input 
                  type="text"
                  required
                  value={newDeal.title}
                  onChange={(e) => setNewDeal({ ...newDeal, title: e.target.value })}
                  className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none transition-all text-right"
                  placeholder="مثال: تخفيضات الشتاء الكبرى"
                />
              </div>
              
              <div className="space-y-1.5">
                <label className="text-sm font-bold text-slate-900 block">وصف وتفاصيل العرض</label>
                <textarea 
                  value={newDeal.description}
                  required
                  onChange={(e) => setNewDeal({ ...newDeal, description: e.target.value })}
                  className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none transition-all h-20 resize-none text-right"
                  placeholder="وصف تفصيلي للخصم وشروط الاستخدام"
                />
              </div>

              {/* Product Association Dropdown */}
              <div className="space-y-1.5">
                <label className="text-sm font-bold text-slate-900 block">ربط العرض بمنتج معين من المتجر (اختياري)</label>
                <select 
                  value={newDeal.productId || ''}
                  onChange={(e) => setNewDeal({ ...newDeal, productId: e.target.value || undefined })}
                  className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none transition-all text-right cursor-pointer"
                >
                  <option value="">-- لا يوجد منتج مرتبط بحد ذاته (عرض عام للقسم/عام) --</option>
                  {products.map(p => (
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-sm font-bold text-slate-900 block">نسبة الخصم (%)</label>
                  <input 
                    type="number"
                    min={1}
                    max={100}
                ...
                <div className="space-y-1.5">
                  <label className="text-sm font-bold text-slate-900 block">كوبون الخصم (اختياري)</label>
                  <input 
                    type="text"
                    value={newDeal.code}
                    onChange={(e) => setNewDeal({ ...newDeal, code: e.target.value.toUpperCase() })}
                    className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none transition-all text-left uppercase"
              ...
              <div className="flex items-center justify-between bg-slate-50 p-4 rounded-xl border border-slate-200">
                <span className="text-sm font-bold text-slate-800">حالة العرض (تفعيل)</span>
                <input 
                  type="checkbox"
              ...
              <div className="space-y-1.5">
                <label className="text-sm font-bold text-slate-900 block">صورة العرض الترويجي</label>
                <div className="flex gap-3">
                  <input 
                    type="text"
                    value={newDeal.image}
                    onChange={(e) => setNewDeal({ ...newDeal, image: e.target.value })}
                    className="flex-1 px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none transition-all text-left"
                    placeholder="https://example.com/image.jpg"
                  />
                  <label className="bg-slate-100 text-slate-900 px-4 py-3 rounded-xl font-bold hover:bg-slate-200 transition-all cursor-pointer flex items-center gap-2">
                    <Plus className="w-5 h-5" />
                    <input 
                      ...
                      onChange={(e) => {
                        ...
                        if (file) {
                          const reader = new FileReader();
                          reader.onloadend = () => {
                            setNewDeal({ ...newDeal, image: reader.result as string });
                          };
                          reader.readAsDataURL(file);
                        }
                      }}
                ...
                {newDeal.image && (
                  <div className="mt-2 relative h-32 rounded-2xl overflow-hidden border border-slate-200">
                    <img src={newDeal.image} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                    <button 
                      onClick={() => setNewDeal({ ...newDeal, image: '' })}
                      className="absolute top-2 right-2 p-1.5 bg-white/95 backdrop-blur rounded-lg text-red-500 shadow-sm"
                    >
                      <X className="w-4 h-4" />
                    </button>
                  </div>
            ...
            <div className="p-8 bg-slate-50 border-t border-slate-100 flex gap-4">
              <button 
                onClick={() => {
                  ...
                  if (editingDeal) {
                    updateDeal({ ...editingDeal, ...newDeal } as Deal);
                  } else {
                    addDeal({
                      id: Math.random().toString(36).substr(2, 9),
                      title: newDeal.title || '',
                      description: newDeal.description || '',
                      discountPercentage: newDeal.discountPercentage || 0,
                      code: newDeal.code || '',
                      image: newDeal.image || '',
                ...
                className="flex-1 bg-indigo-600 text-white py-4 rounded-2xl font-bold shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all"
              >
                {editingDeal ? 'حفظ التغييرات' : 'إضافة العرض'}
              </button>
              <button 
